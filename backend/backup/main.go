...
func main() {
	...
	log.Printf("IMPORTANT: All API endpoints now require authentication")
	log.Printf("IMPORTANT: User settings are loaded from database, not .env file")
...
func registerRoutes(
	...
	// *** CRITICAL FIX: LEGACY API ROUTES NOW REQUIRE AUTHENTICATION ***
	legacyAPI := router.PathPrefix("").Subrouter()
	legacyAPI.Use(authService.Middleware) // Add auth to ALL legacy routes

	// Legacy routes (now protected and using user-specific functions)
	...
	log.Printf("  PUBLIC:  POST /api/auth/register")
	log.Printf("  PUBLIC:  POST /api/auth/login")
	log.Printf("  PROTECTED: All other /api/* routes require Bearer token")
	log.Printf("  PROTECTED: All legacy routes (/, /analyze, /sync, etc.) require Bearer token")
}
...
func loadConfig() {
	...
	log.Printf("Legacy configuration loaded from .env:")
	log.Printf("  Port: %s", config.Port)
	log.Printf("  *** IMPORTANT: API calls now use USER-SPECIFIC settings from database ***")
	log.Printf("  *** Legacy .env values are for backwards compatibility only ***")
}
...
func performSync(operation *sync.SyncOperation, wsManager *sync.WebSocketManager, rollbackService *sync.RollbackService) {
	...
	// TODO: Implement actual sync logic here using user-specific functions
	// This would call your existing sync functions from services.go with user context
	...
	docs := map[string]interface{}{
		...
		"endpoints": map[string]interface{}{
			...
			"settings": map[string]string{
				...
				"GET  /api/settings/asana/projects":    "Get Asana projects (PROTECTED)",
				"GET  /api/settings/youtrack/projects": "Get YouTrack projects (PROTECTED)",
				"POST /api/settings/test-connections":  "Test API connections (PROTECTED)",
			},
			"new_sync": map[string]string{
				"POST /api/sync/start":         "Start sync operation (PROTECTED)",
				"GET  /api/sync/status/{id}":   "Get sync status (PROTECTED)",
				"GET  /api/sync/history":       "Get sync history (PROTECTED)",
				"POST /api/sync/rollback/{id}": "Rollback sync operation (PROTECTED)",
			},
		...
		"authentication": map[string]interface{}{
			...
			"note":   "All endpoints except /health, /api/auth/register, and /api/auth/login require authentication",
		},
		"features": []string{
			"JWT-based authentication",
			"User-specific settings from database",
			"All API calls use user's own Asana PAT and YouTrack credentials",
		...
		"breaking_changes": []string{
			...
			"Each user must configure their own Asana PAT and YouTrack credentials",
		},
		"setup_instructions": []string{
			"1. Register a new user: POST /api/auth/register",
			"2. Login to get JWT token: POST /api/auth/login",
			"3. Configure your settings: PUT /api/settings",
			"4. Test connections: POST /api/settings/test-connections",
			"5. Use legacy endpoints with Authorization header",
		},
	}
